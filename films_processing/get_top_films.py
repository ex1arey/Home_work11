from text_proccesing.unique_counter import word_count
import random
import string
def get_top_films(rating):
    filtered_films = [film for film in imdb_rank if float(film['rating']) >= rating]
    if not filtered_films:
        return None
    return random.choice(filtered_films)['description']
...
film_description = get_top_films(8.5)
unique_word_count = count_unique_words(film_description)

print("Кількість унікальних слів в описі фільму:", unique_word_count)